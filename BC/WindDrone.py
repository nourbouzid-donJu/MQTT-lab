...
def main():
    client = Publish.open_connection("B_C_Wind")
    while True: 
        with open('{}/h1.csv'.format(rootDir), newline='') as csvfile:
            ...
            for row in spamreader:
                if lineCount == 0:
                    ...
                    Publish.publish_data(speedLoad,"sensor/BC/WindSpeed", client) # publish wind speed
                    print("WindDir: " + strWindDir)
                    #print(direction)
                    Publish.publish_data(strWindDir,"sensor/BC/WindDir", client) # publish wind wirection
                    time.sleep(2)
