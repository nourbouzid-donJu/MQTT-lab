...
def main():
    client = Publish.open_connection("C_D_Wind")
    while True: 
        with open('{}/h1.csv'.format(rootDir), newline='') as csvfile:
            ...
            for row in spamreader:
                if lineCount == 0:
                    ...
                    Publish.publish_data(speedLoad,"sensor/CD/WindSpeed", client) # publish wind speed
                    print("WindDir: " + strWindDir)
                    #print(direction)
                    Publish.publish_data(strWindDir,"sensor/CD/WindDir", client) # publish wind wirection
                    time.sleep(2)
